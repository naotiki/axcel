...
import { GuardInfer } from "../../../library/guard/GuardModel";
...
export type MockModel = GuardInfer<typeof mockModel>;
...
export const creator =g.model("Creator",{
  ...
  website:g.string().label("サイトURL").regex({
    regex:/^https?:\/\/.+/,
    regexName:"URL",
...
export const language =g.model("Language",{
  ...
  website:g.string().label("公式サイトURL").regex({
    regex:/^https?:\/\/.+/,
    regexName:"URL",
    hint:"http://かhttps://から始まるURLを入力してください"
  }).optional(),
  creator:g.relation(creator,{
    creatorId:g.int()
  },["id"]),
